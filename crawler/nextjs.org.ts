...
const crawler = new BasicCrawler({
  ...
  async requestHandler({ request, sendRequest, crawler }) {
    ...
    if (await fileExists(absolutePath)) {
      log.info(
        `File ${absolutePath} already exists, reading from disk for link extraction.`,
      );
      ...
      // Check if content is actually HTML (happens if .md version is not available)
...
(async () => {
  // 起始页面 (Next.js App Router 基础)
  const startUrl = "https://nextjs.org/docs/app/getting-started";
